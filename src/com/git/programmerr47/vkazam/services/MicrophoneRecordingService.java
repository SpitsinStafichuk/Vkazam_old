...
/**
 * Base class for both types of recording from microphone: RecordNow - records
 * from microphone now and just one time RecordTimer - records from microphone
 * with fixed delays
 * 
...
public abstract class MicrophoneRecordingService extends RelatingService
		implements GNOperationStatusChanged, GNFingerprintResultReady,
	...
	/**
	 * Flag that tells is service recording fingerprint from microphone now or
	 * not
	 */
	...
	/**
	 * Default recording functionality Simply starts recording and enables
	 * recording flag
	 */
	...
	/**
	 * Default cancel recording functionality Simply cancels recording if
	 * service do this or cancels recognizing if
	 * RecognizeFingerprintService.recognize() is called
	 */
	...
	public void GNResultReady(GNFingerprintResult gnFingerprintResult) {
		...
		if (gnFingerprintResult.isFailure()) {
			onStatusChanged(String.format("[%d] %s",
					gnFingerprintResult.getErrCode(),
					gnFingerprintResult.getErrMessage()));
		} else {
			FingerprintData fingerprint = new FingerprintData(
					gnFingerprintResult.getFingerprintData(), new Date());
			currentRecognizingWrapper = new FingerprintWrapper(fingerprint,
					this, FingerprintWrapper.RECOGNIZE_PRIORITY_HIGHEST);
