...
/**
 * Base logic for working with recognizing. Communicates with Gracenote service
 * and receives answers from it.
 * 
 * Only this service recognize fingerprints. All others recognize services (like
 * MicrophoneRecordingNowService) must translate resulting fingerprint to this
 * service.
 * 
 * @author Michael Spitsin
 * @since 2014-04-12
 */
public class RecognizeFingerprintService extends StartBoundService implements
	...
	/**
	 * Perform recognizing of given fingerprint if there is no finger that
	 * recognizing or put it in queue for recognizing
	 * 
	 * @param wrapper
	 *            - wrapper that contains fingerprint that must be recognized
	 */
	public void recognize(FingerprintWrapper wrapper) {
		if (currentRecognizingFingerprint != null) {
			...
			if (fingerprintQueue.isEmpty()
					|| fingerprintQueue.get(0).getPriority() < wrapper
							.getPriority()) {
	...
	/**
	 * Cancel current recognizing if given fingerprint is send to GraceNote or
	 * remove it from recognizing queue
	 * 
	 * If this fingerprint not found nothing happens
	 * 
	 * @param wrapper
	 *            - wrapper that contains fingerprint that must be canceled
	 */
	...
	public void GNStatusChanged(GNStatus gnStatus) {
		currentRecognizingFingerprint.getFingerprintListener().onStatusChanged(
				gnStatus.getMessage());
	}
	...
	public void GNResultReady(GNSearchResult gnSearchResult) {
		...
		if (gnSearchResult.isFailure()) {
			...
			if (!gnSearchResult.isFingerprintSearchNoMatchStatus()) {
				...
				String coverArtURL = bestResponse.getCoverArt() != null ? bestResponse
						.getCoverArt().getUrl() : null;
				String contributorImageURL = bestResponse.getContributorImage() != null ? bestResponse
						.getContributorImage().getUrl() : null;
				String albumArtist = bestResponse.getAlbumArtist();
				String artistBiographyURL = bestResponse
						.getArtistBiographyUrl();
				...
				songData = new SongData(null, null, trackId, artist, album,
						title, null, null, coverArtURL, new Date(),
						contributorImageURL, artistBiographyURL, songPosition,
						albumReviewUrl, albumReleaseYear, albumArtist);
			}

			currentRecognizingFingerprint.getFingerprintListener()
					.onResultStatus(songData);
		}
	...
	/**
	 * Calls recognizeNow if there is another finger is queue or stops service
	 * working if there is no more fingers
	 */
	...
	private void addFingerprintWrapperToQueue(FingerprintWrapper wrapper) {
		...
		while ((index > -1)
				&& (fingerprintQueue.get(index).getPriority() < wrapper
						.getPriority())) {
			index--;
		}
	...
	private void recognizeNow(FingerprintWrapper wrapper) {
		...
		GNOperations.searchByFingerprint(this, config, wrapper
				.getFingerprintData().getFingerprint());
	}
}
