...
public class SongDAO extends AbstractDAO {
    ...
    public synchronized long insert(Data data) {
        ...
        if (songData.getPleercomUrl() != null) {
            values.put(DBConstants.MUSIC_HISTORY_PLEERCOM_URL, songData.getPleercomUrl());
        }
        if (songData.getVkAudioId() != null) {
            values.put(DBConstants.MUSIC_HISTORY_VK_AUDIO_ID, songData.getVkAudioId());
        }
        if (songData.getContributorImageUrl() != null) {
            values.put(DBConstants.MUSIC_HISTORY_CONTRIBUTOR_IMAGE_URL,
                    songData.getContributorImageUrl());
        }
        if (songData.getArtistBiographyURL() != null) {
            values.put(DBConstants.MUSIC_HISTORY_BIOGRAPHY_URL, songData.getArtistBiographyURL());
        }
        if (songData.getAlbumReviewUrl() != null) {
            values.put(DBConstants.MUSIC_HISTORY_ALBUM_REVIEW_URL, songData.getAlbumReviewUrl());
        }
        if (songData.getAlbumReleaseYear() != null) {
            values.put(DBConstants.MUSIC_HISTORY_ALBUM_RELEASE_YEAR,
                    songData.getAlbumReleaseYear());
        }
        if (songData.getAlbumArtist() != null) {
            values.put(DBConstants.MUSIC_HISTORY_ALBUM_ARTIST, songData.getAlbumArtist());
        }
        if (songData.getPpTitle() != null) {
            values.put(DBConstants.MUSIC_HISTORY_PP_TITLE, songData.getPpTitle());
        }
        if (songData.getPpArtist() != null) {
            values.put(DBConstants.MUSIC_HISTORY_PP_ARTIST, songData.getPpArtist());
        }
    ...
    public synchronized int update(Data data) {
        ...
        int result = database
                .update(DBConstants.MUSIC_HISTORY_TABLE, values, DBConstants.DATE + "=?",
                        new String[]{"" + songData.getDate().getTime()});
    ...
    public synchronized int delete(Data data) {
        ...
        databaseHelper = new DBHelper(context);
        database = databaseHelper.getWritableDatabase();
        int result = database.delete(DBConstants.MUSIC_HISTORY_TABLE, DBConstants.ID + "=?",
                new String[]{"" + songData.getId()});
    ...
    protected List<Data> getListByCursor(Cursor cursor) {
        ...
        for (int i = 0; i < cursor.getCount(); i++) {
            instance = new DatabaseSongData(
                    ...
                    this,
                    cursor.getString(
                            cursor.getColumnIndex(DBConstants.MUSIC_HISTORY_GRACENOTE_TRACK_ID)),
                    ...
                    cursor.getString(
                            cursor.getColumnIndex(DBConstants.MUSIC_HISTORY_COVER_ART_URL)),
                    cursor.getString(
                            cursor.getColumnIndex(DBConstants.MUSIC_HISTORY_CONTRIBUTOR_IMAGE_URL)),
                    cursor.getString(
                            cursor.getColumnIndex(DBConstants.MUSIC_HISTORY_BIOGRAPHY_URL)),
                    cursor.getString(
                            cursor.getColumnIndex(DBConstants.MUSIC_HISTORY_ALBUM_REVIEW_URL)),
                    cursor.getString(
                            cursor.getColumnIndex(DBConstants.MUSIC_HISTORY_ALBUM_RELEASE_YEAR)),
                    cursor.getString(
                            cursor.getColumnIndex(DBConstants.MUSIC_HISTORY_ALBUM_ARTIST)));
            result.add(instance);
            cursor.moveToNext();
        }
        return result;
